...
namespace Ks.Batch.Ftp.Upload
{
    public class FtpClient
    {
        ...
        private static readonly LogWriter Log = HostLogger.Get<FtpClient>();
        ...
        public bool Upload(string remoteFile, string localFile)
        {
            try
            {
                ...
                //FileStream localFileStream = new FileStream(localFile, FileMode.Create);
                FileStream localFileStream = System.IO.File.OpenRead(localFile);
                /* Buffer for the Downloaded Data */
                byte[] byteBuffer = new byte[bufferSize];
                ...
                try
                {
                    Log.InfoFormat("Subiendo archivo: " + localFile);
                    while (bytesSent != 0)
                    {
                    ...
                    Console.WriteLine("");
                    Log.InfoFormat("Subida termninada");
                }
                catch (Exception ex) {
