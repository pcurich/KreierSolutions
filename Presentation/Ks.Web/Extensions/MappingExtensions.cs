...
namespace Ks.Web.Extensions
{
    public static class MappingExtensions
    {
        ...
        /// <param name="cityService">City service (used to prepare a select list). null to don't prepare the list.</param>
            ...
            string overrideAttributesXml = "")
        {
            ...
            if (!excludeProperties && address != null)
            {
                ...
                model.StateProvinceName = address.StateProvince != null ? address.StateProvince.GetLocalized(x => x.Name) : null;
                model.CityId = address.CityId;
                model.CityName = address.City != null ? address.City.GetLocalized(x => x.Name) : null;
            ...
            if (address == null && prePopulateWithCustomerFields)
            {
                ...
                //ignore country, state and city for prepopulation. it can cause some issues when posting pack with errors, etc
                //model.CountryId = customer.GetAttribute<int>(SystemCustomerAttributeNames.CountryId);
                //model.StateProvinceId = customer.GetAttribute<int>(SystemCustomerAttributeNames.StateProvinceId);
                //model.CityId = customer.GetAttribute<int>(SystemCustomerAttributeNames.City);
            ...
            if (addressSettings.CountryEnabled && loadCountries != null)
            {
                ...
                if (addressSettings.StateProvinceEnabled)
                {
                    ...
                    var states = stateProvinceService
                        .GetStateProvincesByCountryId(model.CountryId ?? 0, languageId).ToList();
                    ...
                    if (states.Count > 0)
                    {
                        ...
                        if (addressSettings.CityEnabled)
                        {
                            ...
                            var cities = cityService.GetCitiesByStateProvinceId(model.CityId ?? 0, languageId).ToList();

                            if(cities.Count>0)
                            {
                                model.AvailableCities.Add(new SelectListItem { Text = localizationService.GetResource("Address.SelectCity"), Value = "0" });
                    ...
                    else
                    {
                        ...
                        model.AvailableStates.Add(new SelectListItem
                        {
                            Text = localizationService.GetResource(anyCountrySelected ? "Address.Other" : "Address.SelectState"),
                            Value = "0"
                        });
                    }
                }
