    ...
    public static class HtmlExtensions
    {
            ...
            where TLocalizedModelLocal : ILocalizedModelLocal
        {
            return new HelperResult(writer =>
            {
                ...
                {
                    var storeService = EngineContext.Current.Resolve<IKsSystemService>();
                    if (storeService.GetAllKsSystems().Count >= 2)
                    {
        ...
        public static MvcHtmlString DeleteConfirmation<T>(this HtmlHelper<T> helper, string actionName,
            string buttonsSelector) where T : BaseKsEntityModel
        {
        ...
        public static MvcHtmlString KsLabelFor<TModel, TValue>(this HtmlHelper<TModel> helper, Expression<Func<TModel, TValue>> expression, bool displayHint = true)
        {
            var result = new StringBuilder();
            var metadata = ModelMetadata.FromLambdaExpression(expression, helper.ViewData);
            var hintResource = string.Empty;
            object value;
            if (metadata.AdditionalValues.TryGetValue("KsResourceDisplayName", out value))
            {
                var resourceDisplayName = value as KsResourceDisplayName;
